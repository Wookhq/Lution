...
with gen:
    ...
    st.session_state.rpc = st.toggle(
        LANG["lution.fflags.toggle.rpc"], value=st.session_state.rpc
    )
    st.session_state.fpslimit = st.text_input(
        LANG["lution.fflags.textbox.fpslimit"], st.session_state.fpslimit, max_chars=3
    )
    ...
    st.session_state.lightingtech = st.selectbox(
        ...
        index=[
            ...
            "Shadowmap Lighting (Phase 2)",
            "Future Lighting (Phase 3)",
        ].index(st.session_state.lightingtech),
    )
    # fflags presets
    st.write("fflags presets")
    st.session_state.disablechat = st.toggle(
        LANG["lution.fflags.toggle.bbchat"], value=st.session_state.disablechat
    )
    st.toggle("Disable player shadows", value=st.session_state.disableplayersh)
    st.session_state.fontsize = st.text_input(
        LANG["lution.appearance.textbox.fontsize"],
        value=st.session_state.fontsize,
        max_chars=2,
    )
    ...
    st.caption(
        "NOTE : Not all devices respect MSAA, and some GPUs may still auto-adjust. The flag works best on Windows with Direct3D."
    )
    st.session_state.texturequality = st.selectbox(
        ...
        index=[
            ...
            "Level 3 (High)",
            "Level 4 (Ultra)",
        ].index(st.session_state.texturequality),
    )
    ...
    def reload_fflags():
        st.session_state.fflagseditor = json.loads(
            cs.SplitClientSettingsContent() or "{}"
        )
        st.session_state.fflags_text = json.dumps(
            st.session_state.fflagseditor, indent=4
        )
    ...
    st.caption(
        "The fflags editor slow to update with the config, recommend restart lution then try again"
    )
    with mid:
        st.button(
            "Apply FFlags",
    ...
    st.button(LANG["lution.fflags.button.setupoverlay"], on_click=ff.OverlaySetup)

with log:
    st.header("Player Log / Player Chat Log")
    st.write(
        "To order to use this feature, you have to create a desktop shortcut to Lution."
    )

    st.session_state.plrlogs = st.toggle("Turn on", value=st.session_state.plrlogs)
