...
class Directories:
    ...
    BLOXSTRAP_MOD: Path = (
        Path.home() / "AppData" / "Local" / "Bloxstrap" / "Modifications"
    )
    FISHSTRAP_MOD: Path = (
        Path.home() / "AppData" / "Local" / "Fishstrap" / "Modifications"
    )
...
class Resources:
    ...
    class Navigation:
        class Light:
            MOD_UPDATER: Path = (
                Directories.RESOURCES / "nav" / "light" / "mod_updater.png"
            )
            MOD_GENERATOR: Path = (
                Directories.RESOURCES / "nav" / "light" / "mod_generator.png"
            )
        ...
        class Dark:
            MOD_UPDATER: Path = (
                Directories.RESOURCES / "nav" / "dark" / "mod_updater.png"
            )
            MOD_GENERATOR: Path = (
                Directories.RESOURCES / "nav" / "dark" / "mod_generator.png"
            )
    ...
    class Shortcuts:
        class Light:
            PLACEHOLDER: Path = (
                Directories.RESOURCES / "shortcuts" / "light" / "placeholder.png"
            )

        class Dark:
            PLACEHOLDER: Path = (
                Directories.RESOURCES / "shortcuts" / "dark" / "placeholder.png"
            )
    ...
    class ColorPicker:
        SATURATION_VALUE: Path = (
            Directories.RESOURCES / "color_picker" / "saturation_value.png"
        )
        HUE: Path = Directories.RESOURCES / "color_picker" / "hue.png"
        INDICATOR: Path = Directories.RESOURCES / "color_picker" / "indicator.png"
        INDICATOR_WIDE: Path = (
            Directories.RESOURCES / "color_picker" / "indicator_wide.png"
        )
