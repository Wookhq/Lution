...
class ImageSetData:
    ...
    def __init__(
        self, filepath: Path, directory: Path, icon_sizes: Literal[0, 1, 2, 3] = 0
    ):
        with open(filepath) as file:
            content: str = file.read()

        parsed: dict[str, dict[str, dict[str, str | int]]] = self._parse_file_content(
            content
        )
        ...
        for size, icons in parsed.items():
            ...
            for name, data in icons.items():
                ...
                if imageset_item is None:
                    imageset_item = ImageSet(
                        image_set, directory / f"{image_set}.png", []
                    )
    ...
    def _parse_file_content(
        self, content: str
    ) -> dict[str, dict[str, dict[str, str | int]]]:  # AI-Generated
        icon_map: dict[str, dict[str, dict[str, str | int]]] = {}

        image_size_pattern: str = r"function make_assets_(\dx)\(\).*?(\{.*?\}) end"
        icon_data_pattern: str = (
            r"\['([^']+)'\] = \{ ImageRectOffset = Vector2\.new\((\d+), (\d+)\), ImageRectSize = Vector2\.new\((\d+), (\d+)\), ImageSet = '([^']+)' \}"
        )
