...
def get(
    url: str,
    timeout: int | tuple[int, int] = (5, 10),
    stream: bool = False,
    attempts: int = 3,
    cache: bool = True,
    ...
    for i in range(1, attempts + 1):
        try:
            ...
            Logger.info(
                f"GET {url} -> {response.status_code} {response.reason or 'Reason unknown'} (duration: {duration:.2f}ms)"
            )
            ...
            last_exception = e
            Logger.warning(
                f"GET {url} -> {response.status_code} {response.reason or 'Reason unknown'} (Attempt {i}/{attempts})"
            )

        except ConnectionError as e:
            last_exception = e
            Logger.warning(
                f"GET {url} -> ConnectionError: {e} (Attempt {i}/{attempts})"
            )

        except RequestException as e:
            last_exception = e
            Logger.warning(
                f"GET {url} -> RequestException {type(e).__name__}: {e} (Attempt {i}/{attempts})"
            )

    Logger.error(
        f"GET {url} -> Failed after {attempts} attempt{'' if attempts == 1 else 's'}."
    )
    if last_exception:
